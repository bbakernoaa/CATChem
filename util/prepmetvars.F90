...
module PrepMetVars_Mod
   ...
   public :: INCR_REAL_RANK4, INCR_INT_RANK4
...
CONTAINS
   ...
   SUBROUTINE INCR_INT_RANK4(ARR, RESULT)
      INTEGER, INTENT(IN), TARGET :: ARR(:,:)
      INTEGER, INTENT(INOUT), POINTER :: RESULT(:,:,:,:)

      ALLOCATE(RESULT(1, 1, SIZE(ARR, 1), SIZE(ARR, 2)))
      RESULT(1,1,:,:)=ARR

   END SUBROUTINE INCR_INT_RANK4

   SUBROUTINE INCR_REAL_RANK4(ARR, RESULT)
      REAL, INTENT(IN), TARGET :: ARR(:,:)
      REAL, INTENT(INOUT), POINTER :: RESULT(:,:,:,:)
   ...
   END SUBROUTINE INCR_REAL_RANK4
