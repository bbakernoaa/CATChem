!>ile catchem_wrapper.F90
!!
!!> \defgroup catchem_group CATChem wrapper Module
...
module catchem_wrapper
   ...
   type(GridStateType) :: GridState               !> Grid state object for CATChem
   type(MetStateType), dimension(:) :: MetState   !> Meteorological state array for CATChem
   type(ChemStateType), dimension(:) :: ChemState !> Chemical state array for CATChem
   type(EmisStateType), dimension(:) :: EmisState !> Emission state array for CATChem
   type(DiagStateType), dimension(:) :: DiagState !> Diagnostic state array for CATChem
...
contains
   ...
   !!
   subroutine catchem_wrapper_init(catchem_configfile_in, & !
      do_catchem_in, &
      export_catchem_diags_in, & !> export CATChem diagnostics - Default = .false.
      n_dbg_lines_in, & !> number of debug lines
      im, & !> number of horizontal grid points
      kme, & !> number of vertical grid points
      ...
      ! INPUT PARAMETERS
      !-----------------
      character(len=*), intent(in) :: catchem_configfile_in !> catchem configuration file
      logical, intent(in) :: do_catchem_in                  !> CATChem on off switch - Default = .false.
      logical, intent(in) :: export_catchem_diags_in        !> export CATChem diagnostics - Default = .false.
      integer, intent(in) :: n_dbg_lines_in                 !> number of debug lines
      integer, intent(in) :: im                            !> number of horizontal grid points
      integer, intent(in) :: kme                           !> number of vertical grid points
      ...
      ! LOCAL VARIABLES
      !-----------------
      type(GridStateType) :: GridState_local
      type(MetStateType)  :: MetState_local
      type(ChemStateType) :: ChemState_local
      type(EmisStateType) :: EmisState_local
      type(DiagStateType) :: DiagState_local
      ...
      if (do_catchem_in) then
         ! Get CATChem Configuration yaml file
         ...
         ! TODO : Fix grid state ? do we need it?

         ! Read CATChem Configuration yaml file
         call cc_read_config(Config, MetState(0), EmisState_local, ChemState_local, errflg, CATCHem_ConfigFile)
         if (errflg /= 0) then
            errmsg = 'Error reading CATChem configuration file: '//CATCHem_ConfigFile
            return
         end if

         ! Allocate MetState and set grid information (nsoil, nlevs)
         do i = 1, im
          ! set basic parameters to allocate arrays
          MetState(i)%nLEVS = kme
          MetState(i)%nSOIL = nsoil

          ! Allocate MetState
          call cc_allocate_metstate(MetState(i), errflg)
          if (errflg /= 0) then
             errmsg = 'Error allocating MetState - catchem_wrapper_init'
             return
          end if

          ! Allocate ChemState
          call cc_allocate_chemstate(Config%Species_File, ChemState(i), errflg)
          if (errflg /= 0) then
             errmsg = 'Error allocating ChemState - catchem_wrapper_init'
             return
          end if

          ! Allocate DiagState
          call cc_allocate_diagstate(Config, MetState(i), DiagState(i), errflg)
          if (errflg /= 0) then
             errmsg = 'Error allocating DiagState - catchem_wrapper_init'
             return
          end if

          ! Allocate EmisState
          call cc_allocate_emisstate(EmisState(i), errflg)
          if (errflg /= 0) then
             errmsg = 'Error allocating EmisState - catchem_wrapper_init'
             return
          end if
   ...
   !!
   subroutine catchem_wrapper_finalize()
   ...
   !! \ingroup catchem_wrapper_group
   subroutine ccpp_to_catchem(im, kte, kme, )
