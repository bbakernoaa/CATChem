...
inline int status_code(BoundaryStatus status) { return static_cast<int>(status); }
...
inline void register_builtin_processes() {
    ...
    std::call_once(registered, [] {
        if (catchem_register_carbchem_cpp) catchem_register_carbchem_cpp();
        if (catchem_register_drydep_cpp) catchem_register_drydep_cpp();
        if (catchem_register_dust_cpp) catchem_register_dust_cpp();
        if (catchem_register_seasalt_cpp) catchem_register_seasalt_cpp();
        if (catchem_register_settling_cpp) catchem_register_settling_cpp();
        if (catchem_register_so4chem_cpp) catchem_register_so4chem_cpp();
        if (catchem_register_wetdep_cpp) catchem_register_wetdep_cpp();
    });
}
