...
extern "C" {
    void run_wetdep_science_bridge(
...
void WetDepProcess::run(std::shared_ptr<StateManager> state) {
    ...
    double* pfilsan_ptr  = (pfilsan_it != state->met.fields_3d.end())  ? pfilsan_it->second->host_view.data()  : nullptr;

    auto pfllsan_it  = state->met.fields_3d.find("PFLLSAN");
    double* pfllsan_ptr  = (pfllsan_it != state->met.fields_3d.end())  ? pfllsan_it->second->host_view.data()  : nullptr;

    auto reevapls_it = state->met.fields_3d.find("REEVAPLS");
    double* reevapls_ptr = (reevapls_it != state->met.fields_3d.end()) ? reevapls_it->second->host_view.data() : nullptr;
            ...
            if (meta.is_wetdep) {
                ...
                for (int col = 0; col < state->n_cols; ++col) {
                    for (int lvl = 0; lvl < state->n_levels; ++lvl) {
                        ...
                        if (mass_ptr) mass_ptr[col + lvl * state->n_cols] = diag_mass_bin[idx];
                        if (num_ptr)  num_ptr[col + lvl * state->n_cols]  = diag_flux_bin[idx];
                    }
                }
            }
        }
