...
extern "C" {
    void run_dust_science_bridge(
        int n_cols, int n_levels, int n_species, int n_soil, double dt,
        const char* active_scheme, int diagnostics,
        double* airden, double* clayfrac, double* frlake, double* frsno, double* gvf,
        double* lai, int* lwi, double* rdrag, double* sandfrac, double* soilm,
        double* ssm, double* tskin, double* u10m, double* v10m, double* ustar,
        double* ustar_threshold, double* z0,
        double* species_density, double* species_radius, double* species_lower_radius, double* species_upper_radius,
        double* conc, double* tendency,
        double* diag_emission_total, double* diag_emission_bin, double* diag_horizontal_flux,
        double* diag_moisture_correction, double* diag_effective_threshold, double* diag_utar_threshold,
...
void DustProcess::run(std::shared_ptr<StateManager> state) {
    ...
    double* clayfrac_ptr = (clayfrac_ptr_it != state->met.fields_2d.end()) ? clayfrac_ptr_it->second->host_view.data() : nullptr;
    auto frlake_ptr_it = state->met.fields_2d.find("lake_fraction");
    double* frlake_ptr = (frlake_ptr_it != state->met.fields_2d.end()) ? frlake_ptr_it->second->host_view.data() : nullptr;
    auto frsno_ptr_it = state->met.fields_2d.find("snow_fraction");
    double* frsno_ptr = (frsno_ptr_it != state->met.fields_2d.end()) ? frsno_ptr_it->second->host_view.data() : nullptr;
    ...
    double* rdrag_ptr = (rdrag_ptr_it != state->met.fields_2d.end()) ? rdrag_ptr_it->second->host_view.data() : nullptr;
    auto sandfrac_ptr_it = state->met.fields_2d.find("sand_fraction");
    double* sandfrac_ptr = (sandfrac_ptr_it != state->met.fields_2d.end()) ? sandfrac_ptr_it->second->host_view.data() : nullptr;
    auto soilm_ptr_it = state->met.fields_3d.find("soil_moisture");
    double* soilm_ptr = (soilm_ptr_it != state->met.fields_3d.end()) ? soilm_ptr_it->second->host_view.data() : nullptr;
    ...
    double* tskin_ptr = (tskin_ptr_it != state->met.fields_2d.end()) ? tskin_ptr_it->second->host_view.data() : nullptr;
    auto u10m_ptr_it = state->met.fields_2d.find("u_10m");
    double* u10m_ptr = (u10m_ptr_it != state->met.fields_2d.end()) ? u10m_ptr_it->second->host_view.data() : nullptr;
    auto v10m_ptr_it = state->met.fields_2d.find("v_10m");
    double* v10m_ptr = (v10m_ptr_it != state->met.fields_2d.end()) ? v10m_ptr_it->second->host_view.data() : nullptr;
    auto ustar_ptr_it = state->met.fields_2d.find("friction_velocity");
    double* ustar_ptr = (ustar_ptr_it != state->met.fields_2d.end()) ? ustar_ptr_it->second->host_view.data() : nullptr;
    ...
    if (!airden_ptr) airden_ptr = dummy_1d.data();
    if (!clayfrac_ptr) clayfrac_ptr = dummy_1d.data();
    if (!frlake_ptr) frlake_ptr = dummy_1d.data();
    if (!frsno_ptr) frsno_ptr = dummy_1d.data();
    if (!gvf_ptr) gvf_ptr = dummy_1d.data();
    if (!lai_ptr) lai_ptr = dummy_1d.data();
    if (!lwi_ptr) lwi_ptr = dummy_1d_int.data();
    if (!rdrag_ptr) rdrag_ptr = dummy_1d.data();
    if (!sandfrac_ptr) sandfrac_ptr = dummy_1d.data();
    if (!soilm_ptr) soilm_ptr = dummy_soil.data();
    if (!ssm_ptr) ssm_ptr = dummy_1d.data();
    if (!tskin_ptr) tskin_ptr = dummy_1d.data();
    if (!u10m_ptr) u10m_ptr = dummy_1d.data();
    if (!v10m_ptr) v10m_ptr = dummy_1d.data();
    if (!ustar_ptr) ustar_ptr = dummy_1d.data();
    if (!ustar_th_ptr) ustar_th_ptr = dummy_1d.data();
    if (!z0_ptr) z0_ptr = dummy_1d.data();
    ...
    run_dust_science_bridge(
        state->n_cols, state->n_levels, state->n_species, 4, state->time.timestep, // n_soil=4
        active_scheme.c_str(), diagnostics_enabled ? 1 : 0,
        airden_ptr, clayfrac_ptr, frlake_ptr, frsno_ptr, gvf_ptr, lai_ptr, lwi_ptr,
        rdrag_ptr, sandfrac_ptr, soilm_ptr, ssm_ptr, tskin_ptr, u10m_ptr, v10m_ptr,
        ustar_ptr, ustar_th_ptr, z0_ptr,
        density.data(), radius.data(), lower_radius.data(), upper_radius.data(),
        conc_ptr, mock_tendency.data(),
        diag_emission_total, diag_emission_bin, diag_horizontal_flux,
