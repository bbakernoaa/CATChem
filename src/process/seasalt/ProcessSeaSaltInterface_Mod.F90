...
!! Generation date: 2025-09-12T17:20:29.536452
...
contains
   ...
   subroutine run_geos12_scheme_column(this, column, rc)
      ...
      allocate(aera_m2(1))  ! Surface field - always scalar
      allocate(ssm(1))  ! Surface field - always scalar
      allocate(t(n_levels))  ! Full column atmospheric field
      allocate(u(n_levels))  ! Full column atmospheric field
      allocate(f_under_pbltop(n_levels))  ! Full column atmospheric field
      allocate(pedge(n_levels+1))  ! Full column edge field - requires nz+1 dimensions
      allocate(pedge_dry(n_levels+1))  ! Full column edge field - requires nz+1 dimensions
      allocate(species_density(n_species), species_radius(n_species), species_lower_radius(n_species), species_upper_radius(n_species))
      species_tendencies = 0.0_fp
      ...
      aera_m2(1) = met%AERA_M2  ! Surface field - scalar access
      ssm(1) = met%SSM  ! Surface field - scalar access
      t(1:n_levels) = met%T(1:n_levels)  ! Full column atmospheric field
      u(1:n_levels) = met%U(1:n_levels)  ! Full column atmospheric field
      f_under_pbltop(1:n_levels) = met%F_UNDER_PBLTOP(1:n_levels)  ! Full column atmospheric field
      pedge(1:n_levels+1) = met%PEDGE(1:n_levels+1)  ! Full column edge field - includes all edge levels
      pedge_dry(1:n_levels+1) = met%PEDGE_DRY(1:n_levels+1)  ! Full column edge field - includes all edge levels
   ...
   function get_required_met_fields(this) result(field_names)
      ...
      case ('gong03')
         ...
         field_names(5) = 'V10M'
      case ('geos12')
         allocate(field_names(11))
         field_names(1) = 'FROCEAN'
         field_names(2) = 'FRSEAICE'
         field_names(3) = 'SST'
         field_names(4) = 'USTAR'
         field_names(5) = 'AERA_M2'
         field_names(6) = 'SSM'
         field_names(7) = 'T'
         field_names(8) = 'U'
         field_names(9) = 'F_UNDER_PBLTOP'
         field_names(10) = 'PEDGE'
         field_names(11) = 'PEDGE_DRY'
      case default
