...
void SeaSaltProcess::init(std::shared_ptr<StateManager> state) {
    if (state->diag_mgr) {
        ...
        state->diag_mgr->register_field("seasalt_mass_emission_total", "Total Mass Emission", "kg/m2/s", DiagType::FIELD_2D, dims_1d);
        state->diag_mgr->register_field("seasalt_number_emission_total", "Total Number Emission", "#/m2/s", DiagType::FIELD_2D, dims_1d);
        ...
        for (size_t i = 0; i < state->chem.species_list.size(); ++i) {
            ...
            if (meta.is_seasalt) {
                ...
                state->diag_mgr->register_field(mass_name, "Mass Emission " + meta.short_name, "kg/m2/s", DiagType::FIELD_2D, dims_1d);
                state->diag_mgr->register_field(num_name, "Number Emission " + meta.short_name, "#/m2/s", DiagType::FIELD_2D, dims_1d);
            }
        }
    }
}
...
void SeaSaltProcess::run(std::shared_ptr<StateManager> state) {
    ...
    double* frocean_ptr  = (frocean_it != state->met.fields_2d.end())  ? frocean_it->second->host_view.data()  : nullptr;

    auto frseaice_it = state->met.fields_2d.find("FRSEAICE");
    double* frseaice_ptr = (frseaice_it != state->met.fields_2d.end()) ? frseaice_it->second->host_view.data() : nullptr;
    ...
    if (n_seasalt == 0) return; // No sea salt species configured
    ...
    double* diag_mass_total_ptr = state->diag_mgr ? (double*)state->diag_mgr->get_host_pointer("seasalt_mass_emission_total") : nullptr;
    double* diag_num_total_ptr  = state->diag_mgr ? (double*)state->diag_mgr->get_host_pointer("seasalt_number_emission_total") : nullptr;
    ...
    if (state->diag_mgr && diagnostics_enabled) {
        for (int i = 0; i < n_seasalt; ++i) {
            ...
            for (int col = 0; col < state->n_cols; ++col) {
                if (mass_ptr) mass_ptr[col] = diag_mass_bin[col + i * state->n_cols];
                if (num_ptr)  num_ptr[col]  = diag_num_bin[col + i * state->n_cols];
            }
        }
    }
