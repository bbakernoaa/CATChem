...
inline MechanismDefinition synthetic_mechanism(std::vector<std::string> names,
                                               std::string identity = "synthetic") {
...
public:
    ...
    void init(std::shared_ptr<StateManager>) override {
        ...
        if (fail_init_) throw std::runtime_error("injected init failure");
    }
    void run(std::shared_ptr<StateManager>) override {
        events_.push_back("run:" + name_);
        if (fail_run_) throw std::runtime_error("injected run failure");
    }
