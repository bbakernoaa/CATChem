...
void CatchemPropertiesTest_SpeciesMetadataAPI() {
    ...
    assert(msa_idx > 0);
    // Upstream GOCART carries MSA as a gas in ppmv.  Keep the mechanism
    // metadata aligned with the legacy AQMIO exchange contract.
    assert(catchem_state_is_species_gas(&state, msa_idx) == 1);
    assert(catchem_state_is_species_aerosol(&state, msa_idx) == 0);
